...
const Pagination = ({
  ...
  const buttonClass =
    "grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-slate-200 disabled:hover:bg-white disabled:hover:text-slate-500";
    ...
    <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
        ...
        <label className="inline-flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 font-bold text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
          >
      ...
      <div className="flex items-center justify-between gap-2 sm:justify-end">
        ...
        <span className="min-w-[92px] text-center text-xs font-bold text-slate-600">
          Page <span className="text-indigo-700">{page}</span> of {totalPages}
        </span>
